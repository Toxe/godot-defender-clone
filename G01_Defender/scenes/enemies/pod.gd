...
func swarmer_direction() -> Vector2:
    var direction = Vector2.RIGHT if $MovementComponent.velocity.x >= 0 else Vector2.LEFT
...
func _on_hitbox_component_destroyed():
    for i in range(number_of_swarmers):
        ...
        swarmer_movement_component.velocity = swarmer_direction() * swarmer_movement_component.velocity.length()
