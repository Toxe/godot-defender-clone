...
func wrap_node_around_level(node: Node2D, wrap_to_side: WrapAroundSide, level_width: float) -> void:
    node.global_position.x += level_width * int(wrap_to_side)


func wrap_chunks_around_level(level_chunks: Array[LevelChunk], begin_index: int, end_index: int, wrap_to_side: WrapAroundSide, level_width: float) -> void:
...
func _on_level_chunk_exited(level_chunk: LevelChunk, node: Node2D) -> void:
    var world := get_parent() as World
