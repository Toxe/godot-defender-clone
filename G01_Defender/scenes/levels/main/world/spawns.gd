...
func spawn_humans(count: int, ordered_level_chunks: Array[LevelChunk]) -> void:
...
func spawn_baiter(ordered_level_chunks: Array[LevelChunk]) -> void:
...
func spawn_enemy_wave(num_landers: int, num_bombers: int, num_random_enemies: int, ordered_level_chunks: Array[LevelChunk]) -> void:
...
class RandomSpawnPosition:
    ...
    func _init(pos: Vector2, dist: float) -> void:
        position = pos
...
func position_is_too_close(random_spawn: RandomSpawnPosition, nodes: Array[Node2D]) -> bool:
    return nodes.any(func(other: Node2D) -> bool: return random_spawn.position.distance_to(other.global_position) < random_spawn.min_spawn_distance)
